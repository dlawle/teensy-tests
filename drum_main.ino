...
AudioSynthNoisePink             cymNoise; //xy=100,1035
...
void loop() {
 ...
 if (digitalRead(6) == LOW) {
    ...
    snareLP.frequency(knob3);
    snareSaw.frequency(knob4);
    snareNoise.amplitude(knob5); 
    if (button0.fallingEdge()) {
    ...
    hatLP.frequency(knob3);
    hatPulse.frequency(knob4);
    hatNoise.amplitude(knob5); 
    if (button0.fallingEdge()) {
    ...
    cymLP.frequency(knob3);
    cymPulse.frequency(knob4);
    cymNoise.amplitude(knob5); 
    if (button0.fallingEdge()) {
